...
<?php if($snsf_allow_export == false){?><p>Want to export your list as a XLS to use with Mail Chimp or Constant Contact? How about select custom colors?<a href="http://www.yourlocalwebmaster.com/?product=simple-newsletter-signup-pro" target="_blank">Go Pro today for only $0.99!</a></p>
<?php } else{ 
...
<p>Affordable website design, seo and maintenance @ <a href="http://wwww.yourlocalwebmaster.com">YourLocalWebmaster.com</a></p>
</div>
<?php
}
...
function simple_newsletter_signup_form_shortcode( $atts, $content=null, $code="" ) {
		...
		if(isset($atts['thanks'])){
			...
			$snsf_thank_you ="Cảm ơn bạn đã đăng ký nhận thông tin từ chúng tôi!";
...
if(isset($atts['terms_text'])){
		...
		$list = '<div id="snsf-wrapper"><form method="post" name="simple-newsletter-signup-form" id="snsf-form"><table><thead><tr>Nhập email để nhận thông tin:</tr></thead><tbody><tfoot><tr><td colspan="2"><input type="submit" value="'.$button_title.'"';
		if($terms_url != false){
		$list .= ' disabled ';	
		}
